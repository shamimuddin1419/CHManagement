...
{
    public class InternetBillCollectionController : Controller
    {
        private InternetBillCollectionDA _internetBillCollectionDA;
        public InternetBillCollectionController()
        {
            _internetBillCollectionDA = new InternetBillCollectionDA();
        }
        // GET: InternetBillCollection
        ...
        public ActionResult InternetBillCollectionModified()
        {
            return View();
        }
        ...
        public JsonResult GetBillUnCollectionList(string fromDate, string toDate, int receivedBy)
        {
            try
            {
                List<BillCollectionVM> _objList = _internetBillCollectionDA.GetBillUnCollectionList(DateTime.ParseExact(fromDate, "dd/MM/yyyy", null), DateTime.ParseExact(toDate, "dd/MM/yyyy", null), receivedBy) ?? new List<BillCollectionVM>();
        ...
        public JsonResult UpdateCollectionStatus(List<BillCollectionVM> objs, int userId)
        {
            try
            {
                if (objs.Any())
                {
                    int result = _internetBillCollectionDA.UpdateCollectionStatus(objs,userId);
        ...
        public JsonResult GetPreviousInfoList(int id)
        {
            try
            {
                List<PreviousBillInfoVM> _objList = _internetBillCollectionDA.GetPreviousInfoList(id) ?? new List<PreviousBillInfoVM>();
                ...
                var payedBill = _internetBillCollectionDA.GetLastPaidBill(id);
        ...
        public JsonResult GetBillCollectionList()
        {
            try
            {
                List<BillCollectionVM> _objList = _internetBillCollectionDA.GetBillCollectionList().OrderByDescending(x => x.collectionId).ToList();
        ...
        public JsonResult DeleteCollection(int id)
        {
            try
            {
                ...
                int result = _internetBillCollectionDA.DeleteCollection(id, createdBy);
        ...
        public JsonResult GetMonthlyBill(int cid,string fromMonthYear,string toMonthYear)
        {
            try
            {
                ...
                if (fromMonthYear != "0" && toMonthYear !="0")
                {
                    ...
                    {
                        decimal result = _internetBillCollectionDA.GetMonthlyBill(cid, fromMonthYearDate, toMonthYearDate);
        ...
        public JsonResult GetPages(int yearId)
        {
            try
            {
                List<BillCollectionVM> _objList = _internetBillCollectionDA.GetPagesByYear(yearId) ?? new List<BillCollectionVM>();
                var pageList = _objList
                                .Select(x => new
                                {
                                    id = x.pageNo,
