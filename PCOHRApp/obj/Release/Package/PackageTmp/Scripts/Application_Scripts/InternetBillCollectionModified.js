...
$('#toMonthId').change(function () {
    $.get('InternetBillCollection/GetMonthlyBill?cid=' + $('#customerId').val() + "&fromMonthYear=" + $('#fromMonthId').val() + "&toMonthYear=" + $('#toMonthId').val(), function (data) {
...
$('#btnSaveId').click(function () {
    ...
    //}
        ...
        $.post('/InternetBillCollection/InsertBillCollection', inputData, function (data) {
            if (data.success == true) {
...
function LoadBillDatatable() {
    ...
    $('#customerBillListTableId').DataTable({
        ...
        "ajax":
            {
                "url": '/InternetBillCollection/GetBillCollectionList',
                "type": "POST",
                "datatype": "json"
            },
        //"responsive": true,
...
function ChangeToMonth() {
    ...
    var monthLen = months.length;
    if (monthLen > 0) {
        $.get('/InternetBillCollection/GetMonthlyBill?cid=' + $('#customerId').val() + "&fromMonthYear=" + months[0] + "&toMonthYear=" + months[monthLen - 1], function (data) {
            if (data.success == true) {
                $('#monFeeId').val(data.data);
                calculateAmount();
            }
