...
$('#btnSaveId').click(function () {
    ...
    if ($('#monthId').val() == '0') {
        ...
        if ($('#isBatch').is(":checked") == false && $('#customerId').val() == '0') {
            ...
            $.post('/DishBillGenerate/InsertBillGenerate', inputData, function (data) {
...
function LoadBillDatatable() {
    ...
    $('#customerBillListTableId').DataTable({
        "ajax":
            {
                "url": '/DishBillGenerate/GetBillGenerateList?month='+month+'&year='+year,
                "type": "POST",
                "datatype": "json"
            },
        "responsive": true,
...
function DeleteBill(id) {
    if (confirm("Are you sure want delete the Bill?") == true) {
        $.get('/DishBillGenerate/DeleteBill/' + id, function (data) {
            if (data.success) {
                LoadBillDatatable();
                showSuccessMessage(data.message);
            }
