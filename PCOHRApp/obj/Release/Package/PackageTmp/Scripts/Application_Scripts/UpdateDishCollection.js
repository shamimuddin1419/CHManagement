...
$('#btnSaveId').click(function () {
    if ($('#receivedById').val() == 0) {
        ...
        if (tableData.any()) {
            ...
            $.ajax({
                url: '/DishBillCollection/UpdateCollectionStatus',
                type: 'POST',
...
function loadDataTable() {
    ...
    table = $('#customerBillListTableId').DataTable({
        ...
        "ajax": '/DishBillCollection/GetBillUnCollectionList?fromDate=' + $('#fromDateId input').val() + '&toDate=' + $('#toDateId input').val() + '&receivedBy=' + $('#collectedById').val(),
        "responsive": true,
        "select": true,
        "paging": false,
