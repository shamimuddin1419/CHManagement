...
function loadCustomerList() {
    ...
    $('#customerListTableId').DataTable({
        ...
        "ajax":
            {
                "url": '/InternetCustomer/GetCustomerList',
                "type": "POST",
                "datatype": "json"
            },
        //"responsive": true,
...
$('#customerId').change(function () {
    $.get('/InternetCustomer/GetDuplicateByIdAndSerialPrefix?id=' + $('#customerId').val() + '&serialPrefixId=' + $('#customerSerialPrefixId').val(), function (data) {
...
$('#btnSaveId').click(function () {
    if ($('#customerSerialPrefixId').val() == null || $('#customerSerialPrefixId').val() == '0') {
        ...
        $.post('/InternetCustomer/InsertOrUpdateCustomer', inputData, function (data) {
            if (data.success == true) {
                if ($('#btnSaveId').text().indexOf('Save') >= 0) {
...
function EditCustomer(id) {
    $.get('/InternetCustomer/GetCustomerById/' + id, function (data) {
        if (data.success) {
            loadCreateUI();
