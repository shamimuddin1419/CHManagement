...
function GenerateRDLC(fileType) {
    ...
    var reportType = 'InternetBillCollectionPageGroup';
    if (yearId == "0") {
        ...
        var viewURL = '/CableReport/ShowReport?fileType=' + fileType + '&reportType=' + reportType + '&year=' + yearId + "&pageNo=" + pageNo + '&receivedBy=' + receivedBy + '&topageNo=' + topageNo;
...
function getPagesWhenYearSlc() {
    $.get('/InternetBillCollection/GetPages?yearId=' + $('#yearId').val(), function (data) {
        var pages = data.data;
        $('#frompageNoId').select2({
            data: pages
        });
