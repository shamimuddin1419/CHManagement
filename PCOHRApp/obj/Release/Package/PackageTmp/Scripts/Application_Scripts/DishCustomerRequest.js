...
function loadInitialization() {
    ...
    requestTypeList = [];
    $.get('/Dropdown/GetCustomerRequestTypeList?requestTypeGroup=Dish', function (data) {
...
function onReqTypeChanged() {
    ...
    if (text == 'Bill Upgrade' || text == 'Bill Downgrade') {
...
$('#btnSaveId').click(function () {
    ...
    if ($('#cid').val() == '') {
    ...
    else if ((text == 'Bill Upgrade' || text == 'Bill Downgrade') && $('#updatedMonthlyBillId').val() == '') {
        ...
        else if (text == 'Bill Upgrade' || text == 'Bill Downgrade') {
            inputData = {
                cid: $('#customerId').val(),
                requestTypeId: $('#requestTypeId').val(),
        ...
        $.post('/DishCustomerRequest/InsertCustomerRequest', inputData, function (data) {
...
function DeleteCustomerRequest(id) {
    if (confirm("Are you sure want delete the request?") == true) {
        $.get('/DishCustomerRequest/DeleteCustomerRequest/' + id, function (data) {
            if (data.success) {
                showSuccessMessage(data.message);
                loadCustomerRequestDatatable();
...
function loadCustomerRequestDatatable() {
    ...
    $('#customerRequestListTableId').DataTable({
        "ajax":
            {
                "url": '/DishCustomerRequest/GetCustomerRequestList',
                "type": "POST",
                "datatype": "json"
            },
        "responsive": true,
